...
const ChatMessage: React.FC<ChatMessageProps> = ({ message, onSelectTopic, isDarkMode }) => {
  ...
  // New: Conceptual Section Parser
  const renderConceptualBody = (text: string) => {
    // Regex to find the headers: "1. THE CORE PRINCIPLE:", "2. AN ANALOGY:", "3. THE APPLICATION:"
    const sections = text.split(/(1\. THE CORE PRINCIPLE:|2\. AN ANALOGY:|3\. THE APPLICATION:|4\. CONCEPT MAP:)/g);
    
    if (sections.length <= 1) return <div className="whitespace-pre-wrap">{text}</div>;
      ...
      <div className="space-y-6">
        {sections.map((part, i) => {
          ...
          if (partTrimmed === '1. THE CORE PRINCIPLE:') {
            return (
              <div key={i} className="flex items-center space-x-2 text-indigo-600 dark:text-indigo-400 font-black text-[10px] uppercase tracking-widest mt-4">
          ...
          if (partTrimmed === '2. AN ANALOGY:') {
            return (
              <div key={i} className="flex items-center space-x-2 text-emerald-600 dark:text-emerald-400 font-black text-[10px] uppercase tracking-widest mt-4">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          if (partTrimmed === '3. THE APPLICATION:') {
            return (
              <div key={i} className="flex items-center space-x-2 text-amber-600 dark:text-amber-400 font-black text-[10px] uppercase tracking-widest mt-4">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M13 10V3L4 14h7v7l9-11h-7z" />
                </svg>
                <span>The Direct Answer</span>
              </div>
            );
          }
          if (partTrimmed === '4. CONCEPT MAP:') {
            return (
              <div key={i} className="flex items-center space-x-2 text-rose-600 dark:text-rose-400 font-black text-[10px] uppercase tracking-widest mt-4">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          // Wrap the actual content of the section
          const isInstruction = i > 0 && ['1. THE CORE PRINCIPLE:', '2. AN ANALOGY:', '3. THE APPLICATION:', '4. CONCEPT MAP:'].includes(sections[i-1].trim());
          
          return (
            <div key={i} className={`whitespace-pre-wrap ${isInstruction ? 'pl-6 border-l-2 border-slate-100 dark:border-slate-800' : ''}`}>
              {partTrimmed}
            </div>
          );
        })}
  ...
  const renderBody = () => {
    return (
      <div className="space-y-4">
        ...
        {isAssistant ? renderConceptualBody(mainContent) : <div className="whitespace-pre-wrap">{mainContent}</div>}
      </div>
    );
  };
    ...
    <div className={`flex w-full mb-6 ${isAssistant ? 'justify-start' : 'justify-end'}`} role="article">
      <div className={`flex max-w-[95%] md:max-w-[80%] ${isAssistant ? 'flex-row' : 'flex-row-reverse'}`}>
        <div className={`flex-shrink-0 h-10 w-10 rounded-full flex items-center justify-center text-white font-bold text-sm shadow-sm transition-colors
          ${isAssistant ? 'bg-indigo-600 mr-3' : 'bg-emerald-600 ml-3'}`}>
          {isAssistant ? 'ME' : 'U'}
        </div>
              ...
              <div className="mt-2 pt-2 border-t border-slate-50 dark:border-slate-800 flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  <button
                    ...
                    aria-label="Helpful"
                    className={`p-1 rounded transition-colors ${feedback === 'up' ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-400 dark:text-slate-500 hover:text-indigo-500'}`}
                  >
                  ...
                  <button
                    ...
                    className={`p-1 rounded transition-colors ${isSpeaking ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-400 dark:text-slate-500 hover:text-indigo-500'}`}
                  >
                    {isSpeechLoading ? (
                ...
                <button
                  onClick={handleCopy}
                  className="flex items-center space-x-1 text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500 hover:text-indigo-600 transition-colors"
                >
